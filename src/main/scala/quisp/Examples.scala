...
object Examples {
  ...
  object Radian {
    ...
    def cityTemperatures: Unit = {
      ...
      val months = List("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
        "Dec")
      line(months, List(7.0, 6.9, 9.5, 14.5, 18.2, 21.5, 25.2, 26.5, 23.3, 18.3, 13.9, 9.6))
        ...
        .series(2).name("Berlin")
        .series(2).markerOptions.enabled(true)
        .addSeries(months, List(3.9, 4.2, 5.7, 8.5, 11.9, 15.2, 17.0, 16.6, 14.2, 10.3, 6.6, 4.8))
        .series(3).name("London")
        .legend.orientation(quisp.highcharts.Orientation.vertical)
    ...
    def scratch: Unit = {
      ...
      line(1 to 10)
        .xAxis.label("Dollars")
        .xAxis.range(3, 7)
        .yAxis.label("Doughnuts")
        .legend.borderThickness(3)

      line(1 to 100, (x: Double) => math.sin(x * .04 * math.Pi))
        .defaultLineOptions.strokeColor(Color.RED)
        .defaultLineOptions.strokeWidth(3)
        .defaultMarkerOptions.symbol(Symbol.diamond)
        .defaultMarkerOptions.markerSize(30)
        .xAxis.axisType(AxisType.log)
        .series(0).markerOptions.markerSize(10)
        .series(0).markerOptions.markerFillColor(Color.YELLOW)

      scatter(2 to 20)
        .series(0).lineOptions.strokeColor(Color.BLUE)
        .series(0).markerOptions.markerSize(5)
