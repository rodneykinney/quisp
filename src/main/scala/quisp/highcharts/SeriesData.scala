...
trait SeriesDataConversions {
  ...
  implicit def toDoubleIterable[T <% Double](
      x: Iterable[T]): Iterable[Double] = x.map(_.toDouble)

  implicit def toIterableTupleDouble[T1 <% Double, T2 <% Double](
      x: Iterable[(T1, T2)]): Iterable[(Double, Double)] = x.map { case (a, b) => (a.toDouble, b.toDouble) }

  implicit def toTupleIterableDouble[T1 <% Double, T2 <% Double](
      x: (Iterable[T1], Iterable[T2])): (Iterable[Double], Iterable[Double]) = (x._1.map(_.toDouble), x._2.map(_.toDouble))

  implicit def toIterableTupleStringDouble[T <% Double](
      x: Iterable[(String, T)]): Iterable[(String, Double)] = x.map { case (l, y) => (l, y.toDouble) }

  implicit def toIterableTupleDoubleString[T <% Double](
      x: Iterable[(T, String)]): Iterable[(Double, String)] = x.map { case (y, l) => (y.toDouble, l) }

  implicit def toIterableDoubleFunction[T <% Iterable[Double]](x: (T, Double => Double)): (Iterable[Double], Double
      => Double) = (x._1, x._2)
